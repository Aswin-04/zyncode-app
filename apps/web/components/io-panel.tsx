...
const IOPanel = ({code, input, executionResult, roomId, setInput, setExecutionResult}: IOPanelProps) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className='flex-1 flex flex-col'>
        <Textarea className="flex-1 resize-none" placeholder="Program input..." onChange={(e) => setInput(e.target.value)} />
...
const OutputDisplay = ({ username, stdin, stdout }: {username: string, stdin: string, stdout: string}) => {
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <pre className="bg-muted p-3 rounded text-sm whitespace-pre-wrap">{stdin}</pre>
      </div>
      
      <div>
        <div className="text-sm font-medium mb-2">Output:</div>
        <pre className="bg-muted p-3 rounded text-sm whitespace-pre-wrap">{stdout}</pre>
      </div>
    </div>
  );
};
